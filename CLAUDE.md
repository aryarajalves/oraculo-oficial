...
### Qualidade, Linting e Testes E2E (Obrigatório)
Toda vez que criar uma nova mecânica, código ou for atualizar o projeto no GitHub:
1. **Python (Backend):** Executar `docker exec oraculo_backend ruff check .`
2. **Node.js (Backend):** Executar `docker exec oraculo_backend npm --prefix backend run lint`
3. **React (Frontend):** Executar `npm --prefix frontend run lint` (ou no diretório do frontend)
Todos os erros devem ser corrigidos antes de commitar ou fazer push.

**Refatorações de Código:**
Toda vez que realizar uma refatoração (`refactor:`), você deve obrigatoriamente executar os testes E2E para garantir que a interface continua funcionando sem erros:
- **Testes E2E:** Executar `docker exec oraculo_backend npm --prefix backend run test:e2e`
